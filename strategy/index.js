  ...
  return new Promise((resolve, reject) => {
    readCsvInBatches(
      ...
      (data) => {
        ...
        const date = data[0]?.date ? `${data[0]?.date?.format("MMM DD, YYYY")}` : "";
        process.send({ date, dayPnl: pnl, totalPnl });
        // delay(50);
      },
      () => {
        resolve({
...
(async () => {
  ...
  for (let i = 0; i < SAMPLE_SIZE; i++) {
    const backtestResult = await processFile(`${__dirname}/data_dumps/${FILE_NAME}`);
